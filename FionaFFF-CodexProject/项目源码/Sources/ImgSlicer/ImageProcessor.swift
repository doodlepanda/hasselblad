...
struct ImageProcessor: Sendable {
    ...
    func process(task: FolderTask, settings: CropSettings, sampleProfiles: [SampleProfile] = []) async -> [PhotoProcessResult] {
        ...
        for photo in task.photos {
            ...
            autoreleasepool {
                ...
                let outputs = writeCrops(photo: photo, task: task, regions: regions, settings: settings)
    ...
    func detectCropCandidates(for url: URL, settings: CropSettings, sampleProfiles: [SampleProfile] = []) -> [CropCandidate] {
        ...
        if let blackBorderRegion = detectPreciseSingleBlackBorder(cgImage: cgImage, url: url) {
            let regions = [blackBorderRegion]
            let signature = candidateSignature(regions: regions, marginScale: 0)
            if !signatures.contains(signature) {
                signatures.insert(signature)
                candidates.append(CropCandidate(
                    title: "TIF 黑边精准裁切",
                    detail: "高分辨率自适应黑边检测 · 1 个区域",
                    regions: regions,
                    marginScale: 0,
                    score: 0.995
                ))
    ...
    func detectTemplatePositionRegions(for url: URL, template: CGRect) -> [CropRegion] {
        guard let cgImage = analysisCGImage(for: url, maxPixelSize: 4096),
              let grayImage = grayscaleAnalysisImage(cgImage: cgImage, maxDimension: 4096) else {
            return []
        }

        let gray = grayImage.bytes
        ...
        let rows = projectionSegments(
            ...
            let row = y * width
            for x in 0..<width {
                let value = gray[row + x]
                if value > brightThreshold && value < 248 { content += 1 }
                if y > 0 {
                    edge += abs(Int(gray[row + x]) - Int(gray[(y - 1) * width + x]))
                }
            }
        ...
        for row in filteredRows {
            let columnScores = (0..<width).map { x in
                ...
                for y in row.start..<row.end {
                    ...
                    if value > brightThreshold && value < 248 { content += 1 }
                    if x > 0 {
                        edge += abs(Int(gray[index]) - Int(gray[y * width + x - 1]))
                    }
                }
            ...
            let gapColumns = frameColumnsByDarkGaps(
                scores: columnScores,
            ...
            for column in filteredColumns {
                ...
                let rect = templateRect(center: center, size: template.size)
        ...
        return mergeNormalizedRects(rects, overlapThreshold: 0.72)
            ...
            .enumerated()
            .map { CropRegion(index: $0.offset + 1, rect: $0.element, isManual: true) }
    }

    private func templateRect(center: CGPoint, size: CGSize) -> CGRect {
    ...
    private func projectionSegments(count: Int, minSize: Int, mergeGap: Int, minimumThreshold: Double, score: (Int) -> Double) -> [IntSegment] {
        let raw = (0..<count).map(score)
        return projectionSegments(values: raw, minSize: minSize, mergeGap: mergeGap, minimumThreshold: minimumThreshold)
    }

    private func projectionSegments(values raw: [Double], minSize: Int, mergeGap: Int, minimumThreshold: Double) -> [IntSegment] {
        let count = raw.count
    ...
    private func frameColumnsByDarkGaps(scores rawScores: [Double], templatePixels: Int, imageWidth: Int) -> [IntSegment] {
    ...
    private func splitOverwideSegments(_ segments: [IntSegment], values: [Double], idealSize: Int, gapThreshold: Double) -> [IntSegment] {
    ...
    private func splitSegmentsByExpectedSize(_ segments: [IntSegment], expectedSize: Int) -> [IntSegment] {
    ...
    private func detectPreciseSingleBlackBorder(cgImage: CGImage, url: URL) -> CropRegion? {
        guard isTIFFURL(url) else { return nil }
        guard let grayImage = grayscaleAnalysisImage(cgImage: cgImage, maxDimension: 8192) else { return nil }
        let gray = grayImage.bytes
        let width = grayImage.width
        let height = grayImage.height
        guard width > 80, height > 80, gray.count == width * height else { return nil }

        let threshold = adaptiveBlackThreshold(gray: gray, width: width, height: height)
        let minimumBorder = max(2, min(width, height) / 500)

        let left = firstContentColumn(gray: gray, width: width, height: height, threshold: threshold)
        let right = lastContentColumn(gray: gray, width: width, height: height, threshold: threshold)
        let top = firstContentRow(gray: gray, width: width, height: height, threshold: threshold)
        let bottom = lastContentRow(gray: gray, width: width, height: height, threshold: threshold)

        guard left < right, top < bottom else { return nil }
        guard left >= minimumBorder || top >= minimumBorder || (width - 1 - right) >= minimumBorder || (height - 1 - bottom) >= minimumBorder else {
            return nil
        }

        let refinedLeft = refineVerticalBlackBorderEdge(gray: gray, width: width, height: height, initial: left, side: .before)
        let refinedRight = refineVerticalBlackBorderEdge(gray: gray, width: width, height: height, initial: right, side: .after)
        let refinedTop = refineHorizontalBlackBorderEdge(gray: gray, width: width, height: height, initial: top, side: .before)
        let refinedBottom = refineHorizontalBlackBorderEdge(gray: gray, width: width, height: height, initial: bottom, side: .after)

        guard refinedLeft < refinedRight, refinedTop < refinedBottom else { return nil }
        let rect = CGRect(
            x: Double(refinedLeft) / Double(width),
            y: Double(refinedTop) / Double(height),
            width: Double(refinedRight - refinedLeft + 1) / Double(width),
            height: Double(refinedBottom - refinedTop + 1) / Double(height)
        ).normalized

        guard rect.width >= 0.08, rect.height >= 0.08, rect.width < 0.998 || rect.height < 0.998 else { return nil }
        return CropRegion(index: 1, rect: rect)
    }

    private func grayscaleAnalysisImage(cgImage: CGImage, maxDimension: Int) -> (bytes: [UInt8], width: Int, height: Int)? {
        ...
        let created = gray.withUnsafeMutableBytes { buffer -> Bool in
        ...
        return created ? (gray, width, height) : nil
    }

    private func adaptiveBlackThreshold(gray: [UInt8], width: Int, height: Int) -> UInt8 {
        let insetX = max(1, width / 30)
        let insetY = max(1, height / 30)
        var samples: [UInt8] = []
        samples.reserveCapacity((width * insetY + height * insetX) * 2)

        for y in 0..<height {
            if y < insetY || y >= height - insetY {
                let rowStart = y * width
                for x in 0..<width { samples.append(gray[rowStart + x]) }
            } else {
                let rowStart = y * width
                for x in 0..<insetX { samples.append(gray[rowStart + x]) }
                for x in max(insetX, width - insetX)..<width { samples.append(gray[rowStart + x]) }
            }
        }

        guard !samples.isEmpty else { return 55 }
        samples.sort()
        let p65 = Int(samples[min(samples.count - 1, samples.count * 65 / 100)])
        let p90 = Int(samples[min(samples.count - 1, samples.count * 90 / 100)])
        let threshold = min(96, max(32, min(p90, p65 + 24)))
        return UInt8(threshold)
    }

    private func firstContentColumn(gray: [UInt8], width: Int, height: Int, threshold: UInt8) -> Int {
        let ratios = movingAverage(columnDarkRatios(gray: gray, width: width, height: height, threshold: threshold), window: max(3, width / 600))
        let requiredDrop = 0.68
        for x in 0..<width where ratios[x] < requiredDrop {
            return x
        }
        return 0
    }

    private func lastContentColumn(gray: [UInt8], width: Int, height: Int, threshold: UInt8) -> Int {
        let ratios = movingAverage(columnDarkRatios(gray: gray, width: width, height: height, threshold: threshold), window: max(3, width / 600))
        let requiredDrop = 0.68
        for x in stride(from: width - 1, through: 0, by: -1) where ratios[x] < requiredDrop {
            return x
        }
        return width - 1
    }

    private func firstContentRow(gray: [UInt8], width: Int, height: Int, threshold: UInt8) -> Int {
        let ratios = movingAverage(rowDarkRatios(gray: gray, width: width, height: height, threshold: threshold), window: max(3, height / 600))
        let requiredDrop = 0.68
        for y in 0..<height where ratios[y] < requiredDrop {
            return y
        }
        return 0
    }

    private func lastContentRow(gray: [UInt8], width: Int, height: Int, threshold: UInt8) -> Int {
        let ratios = movingAverage(rowDarkRatios(gray: gray, width: width, height: height, threshold: threshold), window: max(3, height / 600))
        let requiredDrop = 0.68
        for y in stride(from: height - 1, through: 0, by: -1) where ratios[y] < requiredDrop {
            return y
        }
        return height - 1
    }

    private func columnDarkRatios(gray: [UInt8], width: Int, height: Int, threshold: UInt8) -> [Double] {
        var ratios = [Double](repeating: 0, count: width)
        for x in 0..<width {
            var darkCount = 0
            for y in 0..<height where gray[y * width + x] <= threshold {
                darkCount += 1
            }
            ratios[x] = Double(darkCount) / Double(height)
        }
        return ratios
    }

    private func rowDarkRatios(gray: [UInt8], width: Int, height: Int, threshold: UInt8) -> [Double] {
        var ratios = [Double](repeating: 0, count: height)
        for y in 0..<height {
            var darkCount = 0
            let rowStart = y * width
            for x in 0..<width where gray[rowStart + x] <= threshold {
                darkCount += 1
            }
            ratios[y] = Double(darkCount) / Double(width)
        }
        return ratios
    }

    private func refineVerticalBlackBorderEdge(gray: [UInt8], width: Int, height: Int, initial: Int, side: BorderSide) -> Int {
        let radius = max(8, min(64, width / 160))
        let start = max(1, initial - radius)
        let end = min(width - 2, initial + radius)
        var best = initial
        var bestScore = -1.0
        ...
        for x in start...end {
            ...
            for y in 0..<height {
                ...
                score += side == .before ? right - left : left - right
            }
            score /= Double(height * 255)
            if score > bestScore {
                bestScore = score
                best = x
            }
    ...
    private func refineHorizontalBlackBorderEdge(gray: [UInt8], width: Int, height: Int, initial: Int, side: BorderSide) -> Int {
        let radius = max(8, min(64, height / 160))
        let start = max(1, initial - radius)
        let end = min(height - 2, initial + radius)
        var best = initial
        var bestScore = -1.0
        ...
        for y in start...end {
            var score = 0.0
            for x in 0..<width {
                let top = Double(gray[(y - 1) * width + x])
                let bottom = Double(gray[(y + 1) * width + x])
                score += side == .before ? bottom - top : top - bottom
            }
            score /= Double(width * 255)
            if score > bestScore {
                bestScore = score
                best = y
            }
    ...
    private func writeCrops(sourceURL: URL, regions: [CropRegion], settings: CropSettings, rotationDegrees: Double, outputURLForSlice: (Int) -> URL) -> [URL] {
        ...
        guard let source = imageSource(for: sourceURL),
    ...
    private func nextSequentialOutputIndex(in directory: URL, extension outputExtension: String) -> Int {
        ...
        let existing = files.compactMap { url -> Int? in
            guard url.pathExtension.lowercased() == outputExtension.lowercased() else { return nil }
    ...
    private func dustCleaned8BitImage(_ image: CGImage, settings: CropSettings) -> CGImage? {
        ...
        let softBrightFloor = Int(max(118, 156 - strength * 0.28))
        let softContrastThreshold = Int(max(8, 21 - strength * 0.20))
        ...
        if width > 2, height > 2 {
            for y in 1..<(height - 1) {
                ...
                for x in 1..<(width - 1) {
                    ...
                    if value >= Int(brightFloor), contrast >= contrastThreshold, isNeutralBright {
                        let isHardWhiteDefect = channelMin >= 248 && contrast >= contrastThreshold + 8
                        let isStrongNeutralDefect = channelMin >= 180
                            && channelMax - channelMin <= chromaLimit
                            && contrast >= contrastThreshold + 28
                        if texture <= textureLimit
                            || (isHardWhiteDefect && texture <= max(textureLimit + 34, 58))
                            || (isStrongNeutralDefect && texture <= max(textureLimit + 80, 112)) {
                    ...
                    if let lift = localAdditiveRGBLift8(pixels: pixels, width: width, height: height, x: x, y: y),
                       ...
                       lift.spread <= max(10, lift.minimum / 2 + 5) {
                        ...
                        if texture <= max(46, textureLimit + 14) || (isStrongAdditiveWhite && texture <= 120) {
        ...
        let subtleMask = filteredDustMask(
            candidates: subtleCandidates,
            width: width,
            height: height,
            maxSpotArea: min(maxSpotArea, 10),
            maxLineLength: min(maxLineLength, 110),
            slenderLimit: min(slenderLimit, 2)
        )
        for index in mask.indices where subtleMask[index] {
            mask[index] = true
        }
        let bridgedSmoothCandidates = bridgedDustCandidates(smoothBackgroundCandidates, width: width, height: height)
        let smoothMask = filteredDustMask(
            candidates: bridgedSmoothCandidates,
            width: width,
            height: height,
            maxSpotArea: 0,
            maxLineLength: min(maxLineLength, 220),
            slenderLimit: 4,
            minimumScratchLength: 5,
            allowSparseCurves: true
        )
        for index in mask.indices where smoothMask[index] {
            mask[index] = true
        }
        let ridgeMask = filteredDustMask(
            candidates: bridgedDustCandidates(ridgeCandidates, width: width, height: height),
            width: width,
            height: height,
            maxSpotArea: 12,
            maxLineLength: min(maxLineLength, 300),
            slenderLimit: 5,
            minimumScratchLength: 4,
            allowSparseCurves: true
        )
        for index in mask.indices where ridgeMask[index] {
            mask[index] = true
        }
        let texturedCurveMask = filteredDustMask(
            candidates: bridgedDustCandidates(texturedCurveCandidates, width: width, height: height),
            width: width,
            height: height,
            maxSpotArea: 0,
            maxLineLength: min(maxLineLength, 260),
            slenderLimit: 0,
            minimumScratchLength: 8,
            allowSparseCurves: true
        )
        ...
        dustDebug("8bit dust candidates=\(candidates.filter { $0 }.count) subtle=\(subtleCandidates.filter { $0 }.count) smooth=\(smoothBackgroundCandidates.filter { $0 }.count) ridge=\(ridgeCandidates.filter { $0 }.count) mask=\(mask.filter { $0 }.count) size=\(width)x\(height)")
    ...
    private func dustCleaned16BitRGBImage(_ image: CGImage, settings: CropSettings) -> CGImage? {
        ...
        let softBrightFloor = UInt16(max(118, 156 - strength * 0.28) * 257)
        let softContrastThreshold = UInt16(max(8, 21 - strength * 0.20) * 257)
        ...
        if width > 2, height > 2 {
            for y in 1..<(height - 1) {
                ...
                for x in 1..<(width - 1) {
                    ...
                    let texture = localTexture(luma: luma, width: width, height: height, x: x, y: y)
                    if value >= brightFloor, contrast >= contrastThreshold, isNeutralBright {
                        let isHardWhiteDefect = channelMin >= 63736 && contrast >= contrastThreshold + 2056
                        let isStrongNeutralDefect = channelMin >= 46260
                            && channelMax - channelMin <= chromaLimit
                            && contrast >= contrastThreshold + 7196
                        if texture <= textureLimit
                            || (isHardWhiteDefect && texture <= max(textureLimit + 8738, 14906))
                            || (isStrongNeutralDefect && texture <= max(textureLimit + 20560, 28784)) {
                            candidates[index] = true
                        }
                    }
                    if let lift = localAdditiveRGBLift16(pixels: pixels, width: width, height: height, x: x, y: y, littleEndian: littleEndian),
                       ...
                       lift.spread <= max(2570, lift.minimum / 2 + 1285) {
                        ...
                        if texture <= max(11822, textureLimit + 3598) || (isStrongAdditiveWhite && texture <= 30840) {
                            subtleCandidates[index] = true
                        }
                    }
                    let backgroundTexture = localBackgroundTexture(luma: luma, width: width, height: height, x: x, y: y)
        ...
        dustDebug("16bit dust candidates=\(candidates.filter { $0 }.count) subtle=\(subtleCandidates.filter { $0 }.count) smooth=\(smoothBackgroundCandidates.filter { $0 }.count) ridge=\(ridgeCandidates.filter { $0 }.count) mask=\(mask.filter { $0 }.count) size=\(width)x\(height)")
        guard mask.contains(true) else { return image }
    ...
    private func filteredDustMask(candidates: [Bool], width: Int, height: Int, maxSpotArea: Int, maxLineLength: Int, slenderLimit: Int, minimumScratchLength: Int = 1, allowSparseCurves: Bool = false) -> [Bool] {
    ...
    private func localAdditiveRGBLift8(pixels: [UInt8], width: Int, height: Int, x: Int, y: Int) -> (minimum: Int, spread: Int)? {
        var redSum = 0
        var greenSum = 0
        var blueSum = 0
        var count = 0
    ...
    private func localAdditiveRGBLift16(pixels: [UInt8], width: Int, height: Int, x: Int, y: Int, littleEndian: Bool) -> (minimum: UInt16, spread: UInt16)? {
        var redSum = 0
        var greenSum = 0
        var blueSum = 0
        var count = 0
    ...
    private func localBackgroundTexture(luma: [UInt8], width: Int, height: Int, x: Int, y: Int) -> Int {
        var low = 255
        var high = 0
    ...
    private func localBackgroundTexture(luma: [UInt16], width: Int, height: Int, x: Int, y: Int) -> UInt16 {
        var low = 65535
        var high = 0
        for yy in max(0, y - 4)...min(height - 1, y + 4) {
            for xx in max(0, x - 4)...min(width - 1, x + 4) {
    ...
    private func directionalBrightRidge(luma: [UInt8], width: Int, height: Int, x: Int, y: Int) -> Int {
    ...
    private func directionalBrightRidge(luma: [UInt16], width: Int, height: Int, x: Int, y: Int) -> UInt16 {
        let center = Int(luma[y * width + x])
        let directions = [(1, 0), (0, 1), (1, 1), (1, -1)]
        var best = 0
        for (dx, dy) in directions {
    ...
    private func bridgedDustCandidates(_ candidates: [Bool], width: Int, height: Int) -> [Bool] {
    ...
    private func writeTIFFCrop(_ image: CGImage, to url: URL, sourceProperties: [CFString: Any]?) -> Bool {
        ...
        var properties: [CFString: Any] = [
            kCGImagePropertyDepth: image.bitsPerComponent
        ]
    ...
    private func writeJPEGCrop(_ image: CGImage, to url: URL) -> Bool {
