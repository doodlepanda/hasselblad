...
struct SingleFileCropCommand: Sendable {
    let imageURL: URL
    let outputDirectory: URL
    let settings: CropSettings

    static func parse(arguments: [String]) -> SingleFileCropCommand? {
        guard let commandIndex = arguments.firstIndex(of: "--export-single") else { return nil }
        guard arguments.indices.contains(commandIndex + 1) else {
            print("Usage: ImgSlicer --export-single <image-file> [--output-dir folder] [--profile filmScan|gridPhoto|balanced]")
            return nil
        }
        ...
        if let outputIndex = arguments.firstIndex(of: "--output-dir"),
           arguments.indices.contains(outputIndex + 1) {
            outputDirectory = URL(fileURLWithPath: arguments[outputIndex + 1]).standardizedFileURL
        } else {
            outputDirectory = URL(fileURLWithPath: "/private/tmp/imgslicer-single-output", isDirectory: true)
        }

        return SingleFileCropCommand(imageURL: imageURL, outputDirectory: outputDirectory, settings: settings)
    }

    func run() {
        let start = Date()
        let processor = ImageProcessor()
        let result = processor.processSingleFile(url: imageURL, outputDirectory: outputDirectory, settings: settings)
        let elapsed = Date().timeIntervalSince(start)
        print("\(imageURL.lastPathComponent)\tregions=\(result.regions.count)\tcandidates=\(result.candidates.count)\toutputs=\(result.outputURLs.count)\ttime=\(String(format: "%.3f", elapsed))s")
...
struct TemplateDetectionCommand: Sendable {
    let imageURL: URL
    let template: CGRect

    static func parse(arguments: [String]) -> TemplateDetectionCommand? {
        guard let commandIndex = arguments.firstIndex(of: "--detect-template") else { return nil }
        guard arguments.indices.contains(commandIndex + 1) else {
            print("Usage: ImgSlicer --detect-template <image-file> [--template x,y,w,h]")
            return nil
        }
        let imageURL = URL(fileURLWithPath: arguments[commandIndex + 1]).standardizedFileURL
        var template = CGRect(x: 0.02, y: 0.05, width: 0.16, height: 0.35)
        if let templateIndex = arguments.firstIndex(of: "--template"),
           arguments.indices.contains(templateIndex + 1) {
            let values = arguments[templateIndex + 1].split(separator: ",").compactMap { Double($0) }
            if values.count == 4 {
                template = CGRect(x: values[0], y: values[1], width: values[2], height: values[3])
            }
        }
        return TemplateDetectionCommand(imageURL: imageURL, template: template)
    }

    func run() {
        let start = Date()
        let regions = ImageProcessor().detectTemplatePositionRegions(for: imageURL, template: template)
        let elapsed = Date().timeIntervalSince(start)
        print("\(imageURL.lastPathComponent)\tregions=\(regions.count)\ttime=\(String(format: "%.3f", elapsed))s")
        for region in regions {
            let rect = region.rect
            print("\(region.index)\t\(String(format: "%.5f", rect.minX)),\(String(format: "%.5f", rect.minY)),\(String(format: "%.5f", rect.width)),\(String(format: "%.5f", rect.height))")
        }
    }
}
...
private func applyCLISettings(arguments: [String], settings: inout CropSettings) {
    ...
    if arguments.contains("--dust") {
        settings.dustRemovalEnabled = true
    }

    if let dustIndex = arguments.firstIndex(of: "--dust-strength"),
       arguments.indices.contains(dustIndex + 1),
       let strength = Double(arguments[dustIndex + 1]) {
        settings.dustRemovalStrength = max(0, min(100, strength))
        settings.dustRemovalEnabled = strength > 0
    }
}
...
private extension CropAlgorithmMode {
    init?(cliValue: String) {
        ...
        case "automatic", "auto":
            self = .automatic
        case "lightGaps", "light", "gaps", "negative":
            self = .lightGaps
