...
final class AppStore: ObservableObject {
    ...
    func updateSelectedCrop(regionID: CropRegion.ID, rect: CGRect, angle: Double? = nil, manual: Bool = true) {
        ...
        templateImageAspect = imageAspect(url: tasks[indexes.task].photos[indexes.photo].url)
    ...
    private func taskWithTemplateAppliedIfAvailable(_ task: FolderTask) -> FolderTask {
        ...
        guard isTemplateCompatible(with: task) else { return task }
        var prepared = task
    ...
    private func isTemplateCompatible(with task: FolderTask) -> Bool {
        guard let templateImageAspect,
              let firstPhoto = task.photos.first,
              let currentAspect = imageAspect(url: firstPhoto.url) else {
            return true
        }
