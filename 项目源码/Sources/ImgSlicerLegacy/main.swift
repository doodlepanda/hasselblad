...
final class LegacyWindowController: NSViewController {
    ...
    @objc private func importFile() {
        ...
        panel.allowedFileTypes = ["tif", "tiff", "jpg", "jpeg", "png", "bmp"]
    ...
    @objc private func autoIdentify() {
        ...
        DispatchQueue.global(qos: .userInitiated).async {
            let results = photos.map { photo -> [LegacyCrop] in
                let detected = LegacyFrameDetector.detectBestTemplateCrops(url: photo.url, template: template)
                return detected.isEmpty ? LegacyFrameDetector.tiledCrops(template: template) : detected
            }
            DispatchQueue.main.async {
                guard self.tasks.indices.contains(taskIndex) else { return }
                for index in self.tasks[taskIndex].photos.indices {
...
final class LegacyCanvasView: NSView {
    ...
    override func draw(_ dirtyRect: NSRect) {
        ...
        for (index, crop) in crops.enumerated() {
            ...
            path.stroke()
            let handle = rotateHandlePoint(rect: r, angle: crop.angle)
            NSColor.red.setFill()
    ...
    override func mouseDragged(with event: NSEvent) {
        ...
        if activeHandle == .rotate {
            ...
            crops[activeIndex].angle = startAngle + current - start
            onCropsChanged?()
            needsDisplay = true
            return
        }
    ...
    private func rotateHandlePoint(rect: CGRect, angle: CGFloat) -> CGPoint {
        rotate(point: CGPoint(x: rect.minX - 14, y: rect.minY - 14), around: CGPoint(x: rect.midX, y: rect.midY), angle: angle)
    }

    private func rotate(point: CGPoint, around center: CGPoint, angle: CGFloat) -> CGPoint {
    ...
    private func hitHandle(point: CGPoint, rect: CGRect, angle: CGFloat) -> CropHandle? {
        ...
        let edge: CGFloat = 24
        let rotatePoint = rotateHandlePoint(rect: rect, angle: angle)
