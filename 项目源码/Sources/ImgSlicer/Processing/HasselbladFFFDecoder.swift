...
final class HasselbladFFFDecoder {
    ...
    private func readRow(_ row: Int, handle: FileHandle, expectedBytes: Int) -> Data? {
        ...
        do {
            try handle.seek(toOffset: rowOffset)
            return try handle.read(upToCount: expectedBytes)
        } catch {
            return nil
        }
    }
    ...
    private static func readIFD(handle: FileHandle, offset: UInt64, endian: Endian) -> [IFDEntry]? {
        do {
            ...
            guard let entryData = try handle.read(upToCount: count * 12), entryData.count == count * 12 else { return nil }
    ...
    private static func nextIFDOffset(handle: FileHandle, offset: UInt64, endian: Endian) -> UInt64? {
        do {
            ...
            try handle.seek(toOffset: offset + 2 + count * 12)
            guard let nextData = try handle.read(upToCount: 4), nextData.count == 4 else { return nil }
    ...
    private static func valueData(entry: IFDEntry, handle: FileHandle) -> Data? {
        ...
        do {
            try handle.seek(toOffset: UInt64(entry.valueOffset))
